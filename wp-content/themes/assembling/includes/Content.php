...
class Content {
	...
	private function video() {
		$cycle = get_sub_field('cycle');

		$title = get_sub_field('content-section-title');
		$video = get_sub_field('video-link');
		$autoplay = get_sub_field('autoplay');
		$hideOverlay = get_sub_field('shov_overlay');

		$dopString = "?enablejsapi=1";

		if($autoplay) {
			$dopString = "?autoplay=1&mute=1";
		}

		if($cycle) {
			$pos = strpos($video, '=', 1); // $pos = 7, not 0
			$rest = substr($video, $pos+1);    // returns "ef"
			$dopString = $dopString."&playlist=".trim($rest);
		}

		if($hideOverlay) {
			$dopString = $dopString."&controls=0";
		}
			...
			<div class="content-video">
				<div class="container">';
					...
					echo '<div class="video-wrap">';
						if(!$autoplay) echo '<div class="video-preview" style="background-image: url(' . $preview . '); background-size: cover;"> <a href="javascript:void(0)" class="video__button-play">' . get_field('watch', 'option') . '</a>';
